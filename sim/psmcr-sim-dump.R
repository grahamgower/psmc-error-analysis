...
for (m in models) {
	for (x in fragsizes) {
		for (p in 1:npops) {
			for (s in 1:nsamples) {
				psmc_file <- sprintf("out/m%d/psmc/p%d/s%d/x%s/psmc.out", m, p, s-1, x)
				psmc <- lastround(parse_psmc(psmc_file, mu, g))
				d = rbind(d, cbind(prog="PSMC", model=m, fragsize=as.integer(x), sample=s, population=p, psmc))

				psmc_file <- sprintf("out/m%d/psmc-trunc/p%d/s%d/x%s/psmc.out", m, p, s-1, x)
				psmc <- lastround(parse_psmc(psmc_file, mu, g))
				d = rbind(d, cbind(prog="PSMC-trunc", model=m, fragsize=as.integer(x), sample=s, population=p, psmc))

				msmc1_filebase <- sprintf("out/m%d/msmc1/p%d/s%d/x%s/msmc1.out", m, p, s-1, x)
				msmc1 = lastround(parse_msmc(msmc1_filebase, mu, g))
				d = rbind(d, cbind(prog="MSMC1", model=m, fragsize=as.integer(x), sample=s, population=p, msmc1))
			}
		}
	}
}
